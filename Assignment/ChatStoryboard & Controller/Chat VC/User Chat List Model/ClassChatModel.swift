...
class ClassChatModel : NSObject, NSCoding{

    var message : [ClassChatModelMessage]!
    var statusCode : Int!
    ...
    init(fromDictionary dictionary: [String:Any]){
        ...
        message = [ClassChatModelMessage]()
        if let messageArray = dictionary["message"] as? [[String:Any]]{
            for dic in messageArray{
                let value = ClassChatModelMessage(fromDictionary: dic)
    ...
    {
        message = aDecoder.decodeObject(forKey: "message") as? [ClassChatModelMessage]
